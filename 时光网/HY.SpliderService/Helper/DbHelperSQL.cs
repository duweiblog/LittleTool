...
namespace HY.SpiderService //可以修改成实际项目的命名空间名称
{
    /// <summary>
    /// Copyright (C) 2007-2010 
    /// 数据访问抽象基础类(基于SQLServer)
    /// 用户可以修改满足自己项目的需要。
    /// </summary>
    public class DbHelperSQL
    {
        ...
        public DbHelperSQL(string _connectionstring)
        {
            connectName = _connectionstring;
        ...
        /// <summary>
        /// 连接标准数据库,连接字符串在config文件中
        /// 参数：IsLockDB 等于ture连接锁库，false连接标准库
        /// </summary>
        /// <param name="IsLockDB">ture连接锁库，false连接标准库</param>
        /// <returns></returns>
        public string GetConnection(bool IsLockDB)
        {
            return ConfigurationOperator.GetValue(connectName);
        }
        ...
        public int ExecuteSql(string SQLString, bool IsLockDB)
        {
            using (SqlConnection connection = new SqlConnection(GetConnection(IsLockDB)))
            {
                using (SqlCommand cmd = new SqlCommand(SQLString, connection))
                {
                    try
        ...
        public void BulkToDB(string TableName, DataTable dt)
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                try
                {
                    SqlBulkCopy bulkCopy = new SqlBulkCopy(conn, SqlBulkCopyOptions.CheckConstraints, tx);
        ...
        public object GetSingleVal(string SQLString)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand cmd = new SqlCommand(SQLString, connection))
        ...
        /// <param name="strSQL">查询语句</param>
        /// <returns>SqlDataReader</returns>
        public SqlDataReader ExecuteReader(string strSQL, bool IsLockDB)
        {
            //SqlConnection connection = new SqlConnection(GetConnection(IsLockDB));
            if (conn.State == ConnectionState.Open)
                conn.Close();
            conn.ConnectionString = GetConnection(IsLockDB);
        ...
        public void ExecuteSqlTran(string SQLStringList)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                try
                {
                    foreach (string text in SQLStringList.Split(new char[] { ';' }))
                    {
                        if (text.Trim() != "")
                        {
        ...
        private void PrepareCommand(SqlCommand cmd, SqlConnection conn, SqlTransaction trans, string cmdText, CommandType cmdType, SqlParameter[] cmdParms)
        {
            if (conn.State != ConnectionState.Open)
                conn.Open();
            cmd.Connection = conn;
        ...
        /// <returns>查询结果集，类型为DataTable</returns>
        public DataTable ExecuteProcedure(CommandType cmdType, string cmdText, params SqlParameter[] cmdParms)
        {
            ...
            try
            {
                PrepareCommand(cmd, connection, null, cmdText, cmdType, cmdParms);
        ...
        /// <returns></returns>
        public List<T> DataTableToObjList<T>(DataTable dt) where T : new()
        {
