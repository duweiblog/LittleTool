...
namespace HY.XieHuiImport
{
    public class ImportArticle
    {
        ...
        {
            var dt = DbHelperSQL.Query("select * from KS_Article where 1!=1").Tables[0];  
            var dateNow = DateTime.Now;

            var orderID = int.Parse(DbHelperSQL.GetSingleVal("SELECT isnull(MAX(OrderId),0) OrderId FROM dbo.KS_Article  WHERE TID='20144584893132'").ToString()); 
            var index = 0;
            foreach (var item in notices)
            {
                ++index;
                DataRow dr = dt.NewRow();
                dr["TID"] = "20144584893132"; 
                dr["Title"] = item.Title;
                dr["Intro"] = item.Intro;
            ...
            DbHelperSQL.BulkToDB("KS_Article", dt);  
        }
